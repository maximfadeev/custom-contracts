...
class Contract {
  constructor(contractInfo) {
    let { contractName, tokenName, royalties } = contractInfo;
    if (!contractName) throw "No contract name provided";
    if (!tokenName) throw "No token name provided";
    const royaltyData = this.generateRoyaltyData(royalties);

    return {
      contractName: contractName,
      contractClarity: this.generateContractCode(tokenName, royaltyData),
    };
  }

  generateRoyaltyData(royalties) {
    let totalRoyaltyPercentage = 10;
    let royaltiesFormatted = [];
    for (let i = 0; i < royalties.length; i += 2) {
      const recipient = royalties[i];
      if (!recipient) throw "No royalty recipient provided";
      const percentage = royalties[i + 1];
      if (!percentage) throw "No royalty percentage provided";
      const percentageInt = Number.parseInt(percentage);
      if (isNaN(percentageInt)) throw "Royalty percentage is NaN";
      if (percentageInt <= 0) throw "Royalty percentage cannot be less than or equal to 0";
      totalRoyaltyPercentage += percentageInt;
      const percentageUint = "u" + percentageInt;
      royaltiesFormatted.push([recipient, percentageUint]);
    }
    if (totalRoyaltyPercentage >= 100) throw "Total royalties must be less than 100";
    const ownerPercentage = 100 - totalRoyaltyPercentage;
    return {
      royalties: royaltiesFormatted,
      ownerPercentage: "u" + ownerPercentage,
    };
  }

  generateRoyaltiesClarity(royalties) {
    let royaltyClarity = "";
    for (let [recipient, percentage] of royalties) {
      royaltyClarity += this.formatRoyalty(recipient, percentage);
    }
    return royaltyClarity;
  }

  formatRoyalty(royaltyRecipient, royaltyPercentage) {
    return `\t\t(try! (pay-share (/ (* token-price ${royaltyPercentage}) u100) '${royaltyRecipient}))\n`;
  }

  generateContractCode(tokenName, royaltyData) {
    return `(impl-trait 'SP248HH800501WYSG7Z2SS1ZWHQW1GGH85ME34NT2.nft-trait.nft-trait)
(define-non-fungible-token ${tokenName} uint)
...
(define-private (mint (all-token-data {token-id: uint, minter: principal, metadata: (string-ascii 256), data: {price: uint, for-sale: bool}}))
  (let 
    ...
    (try! (nft-mint? ${tokenName} token-id minter))
...
(define-private (mint-transfer (file {all-token-data: {token-id: uint, minter: principal, metadata: (string-ascii 256), data: {price: uint, for-sale: bool}}, recipient: principal}))
  (let (
    ...
    (match (nft-transfer? ${tokenName} token-id minter recipient)
...
(define-private (delete-nft (token-id uint))
  (begin
    ...
    (try! (nft-burn? ${tokenName} token-id (unwrap! (nft-get-owner? ${tokenName} token-id) ERR-TOKEN-DNE)))
    (ok token-id)
  )
)
...
(define-private (pay-transfer (token-id uint) (token-owner principal) (token-recipient principal))
  (let ((token-price (get price (unwrap! (map-get? token-data token-id) ERR-TOKEN-DNE))))
    ...
    (try! (nft-transfer? ${tokenName} token-id token-owner token-recipient))
...
${this.generateRoyaltiesClarity(royaltyData.royalties)}\t\t(try! (pay-share (/ (* token-price ${
      royaltyData.ownerPercentage
    }) u100) token-owner)) ;; Owner share
    (ok true)
  )
)
  ...
  (let (
    ...
    (if (is-none token-recipient-optional)
      ...
      (let ((token-recipient (unwrap! token-recipient-optional ERR-UNWRAPPING)))
        (try! (nft-transfer? ${tokenName} token-id tx-sender token-recipient))
        (ok token-id)
      )
    )
  )
...
(define-public (purchase (token-id uint))
  (let 
    (
      ...
      (token-owner (unwrap! (nft-get-owner? ${tokenName} token-id) ERR-TOKEN-DNE))
    )
    (asserts! is-token-for-sale ERR-TOKEN-NOT-FOR-SALE)
    (try! (pay token-price token-owner))
    (try! (nft-transfer? ${tokenName} token-id token-owner tx-sender))
    (ok (map-set token-data token-id {for-sale: false, price: token-price}))
  )
)
...
(define-public (set-token-price-data (token-id uint) (price uint) (for-sale bool))
  (begin 
    (asserts! (is-eq (some tx-sender) (nft-get-owner? ${tokenName} token-id)) ERR-NOT-AUTHORIZED)
...
(define-public (delete-token (token-id uint))
  (let ((nft-owner (unwrap! (nft-get-owner? ${tokenName} token-id) ERR-TOKEN-DNE)))
...
(define-public (transfer (token-id uint) (owner principal) (recipient principal))
  (begin 
    (asserts! (is-eq (some tx-sender) (nft-get-owner? ${tokenName} token-id)) ERR-NOT-AUTHORIZED)
    (try! (nft-transfer? ${tokenName} token-id owner recipient))
...
(define-read-only (get-all-token-data (token-id uint))
  (ok {
      ...
      token-owner: (nft-get-owner? ${tokenName} token-id),
    })
)
  
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ${tokenName} token-id)))
...
function createContract(contractInfo) {
  try {
    ...
    const newContractJSON = JSON.stringify(newContract);
    console.log(newContractJSON);

    // fs.writeFile(`${newContract.contractName}.clar`, newContract.contractClarity, (err) => {
    //   if (err) {
    //     console.error(err);
    //   }
...
function main() {
  ...
  const [contractName, tokenName, ...royalties] = args;
  createContract({ contractName, tokenName, royalties });
}
